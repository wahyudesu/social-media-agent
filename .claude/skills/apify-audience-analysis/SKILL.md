...
```bash
export $(grep APIFY_API_TOKEN .env | xargs) && mcpc --json mcp.apify.com --header "Authorization: Bearer $APIFY_API_TOKEN" tools-call fetch-actor-details actor:="ACTOR_ID" | jq -r ".content"
```
...
```bash
node --env-file=.env .claude/skills/apify-audience-analysis/reference/scripts/run_actor.js \
  --actor "ACTOR_ID" \
  --input 'JSON_INPUT'
```
...
`APIFY_API_TOKEN not found` - Ask user to create `.env` with `APIFY_API_TOKEN=your_token`
